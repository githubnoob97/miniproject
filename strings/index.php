<?php
$wambuaCorruptedFile = "N3AmE,JO9b TitlE32s,Department,Full or= Pa+rt-Time,Salary or Hourly,Typi4>/>cal Hours,Annual Sal>ary,Hou)(rly Rate
...
WALK></OSZ,   KECAJ,POLICE OFFICER,POLICE,F,Salary,,KES90024090,";
  function cleanString($value){
    $characters = array("+","%","^","/","?","-",")","£","$",">","@","(","=","&",";");
    $value = str_replace($characters,null,$value);
    return $value;
  }
  $explodeArray = explode(',',$wambuaCorruptedFile);
  //$toparse = cleanString($explodeArray);
  $chunkedArray = array_chunk($explodeArray,8);
  $x = sizeof($chunkedArray);
  echo "<table border='1' style = 'border: 2px solid black;'>";
  // This is the first for loop
  for ($i=0; $i < sizeof($chunkedArray); $i++) {
    echo "<tr>";
    $row = $chunkedArray[$i];
    // This is the second for loo
